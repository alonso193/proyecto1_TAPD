...
void isrTimer_SetVector(cyisraddress address)
{
    ...
    ramVectorTable[CYINT_IRQ_BASE + (uint32)isrTimer__INTC_NUMBER] = address;
}
...
cyisraddress isrTimer_GetVector(void)
{
    ...
    return ramVectorTable[CYINT_IRQ_BASE + (uint32)isrTimer__INTC_NUMBER];
}
...
{
    *isrTimer_INTC_PRIOR = priority << 5;
}
...
uint8 isrTimer_GetPriority(void)
{
    ...
    priority = *isrTimer_INTC_PRIOR >> 5;
