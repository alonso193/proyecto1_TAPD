...
void ToneClock_Start(void) 
{
    ...
    ToneClock_CLKEN |= ToneClock_CLKEN_MASK;
	ToneClock_CLKSTBY |= ToneClock_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: ToneClock_Stop
********************************************************************************
*
* Summary:
...
void ToneClock_Stop(void) 
{
    ...
    ToneClock_CLKEN &= (uint8)(~ToneClock_CLKEN_MASK);
	ToneClock_CLKSTBY &= (uint8)(~ToneClock_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: ToneClock_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((ToneClock_CLKEN & ToneClock_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = ToneClock_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = ToneClock_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* ToneClock__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(ToneClock_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(ToneClock_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: ToneClock_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: ToneClock_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = ToneClock_GetSourceRegister();
    uint16 oldDivider = ToneClock_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = ToneClock_CLKEN & ToneClock_CLKEN_MASK;
        ...
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((ToneClock_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: ToneClock_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(ToneClock_DIV_PTR);
}


/*******************************************************************************
* Function Name: ToneClock_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    ToneClock_MOD_SRC |= modeBitMask & (uint8)ToneClock_MODE_MASK;
}


/*******************************************************************************
* Function Name: ToneClock_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    ToneClock_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(ToneClock_MODE_MASK));
}


/*******************************************************************************
* Function Name: ToneClock_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return ToneClock_MOD_SRC & (uint8)(ToneClock_MODE_MASK);
}


/*******************************************************************************
* Function Name: ToneClock_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = ToneClock_GetDividerRegister();
    uint8 oldSrc = ToneClock_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        ToneClock_MOD_SRC |= CYCLK_SSS;
        ToneClock_MOD_SRC =
            (ToneClock_MOD_SRC & (uint8)(~ToneClock_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: ToneClock_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return ToneClock_MOD_SRC & ToneClock_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: ToneClock_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    ToneClock_PHASE = clkPhase & ToneClock_PHASE_MASK;
}


/*******************************************************************************
* Function Name: ToneClock_GetPhase
********************************************************************************
*
* Summary:
...
{
    return ToneClock_PHASE & ToneClock_PHASE_MASK;
}
